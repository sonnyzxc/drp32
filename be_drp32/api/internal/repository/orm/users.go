...
func (r *userR) GetRewards() RewardSlice {
	if r == nil {
		return nil
	}
	return r.Rewards
...
func (o *User) Rewards(mods ...qm.QueryMod) rewardQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"redeemed\" on \"rewards\".\"reward_id\" = \"redeemed\".\"reward_id\""),
		qm.Where("\"redeemed\".\"user_id\"=?", o.UserID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRewards(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.Select("\"rewards\".\"reward_id\", \"rewards\".\"description\", \"rewards\".\"points\", \"a\".\"user_id\""),
		qm.From("\"rewards\""),
		qm.InnerJoin("\"redeemed\" as \"a\" on \"rewards\".\"reward_id\" = \"a\".\"reward_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.RewardID, &one.Description, &one.Points, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for rewards")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice rewards")
		}
...
func (o *User) AddRewards(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Reward) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"redeemed\" (\"user_id\", \"reward_id\") values ($1, $2)"
		values := []interface{}{o.UserID, rel.RewardID}
...
// SetRewards removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Users's Rewards accordingly.
// Replaces o.R.Rewards with related.
// Sets related.R.Users's Rewards accordingly.
func (o *User) SetRewards(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Reward) error {
	query := "delete from \"redeemed\" where \"user_id\" = $1"
	...
	removeRewardsFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Rewards = nil
	}

	return o.AddRewards(ctx, exec, insert, related...)
}

// RemoveRewards relationships from objects passed in.
// Removes related items from R.Rewards (uses pointer comparison, removal does not keep order)
// Sets related.R.Users.
func (o *User) RemoveRewards(ctx context.Context, exec boil.ContextExecutor, related ...*Reward) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"redeemed\" where \"user_id\" = $1 and \"reward_id\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.UserID}
	for _, rel := range related {
		values = append(values, rel.RewardID)
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.Rewards {
			...
			ln := len(o.R.Rewards)
			if ln > 1 && i < ln-1 {
				o.R.Rewards[i] = o.R.Rewards[ln-1]
			}
			o.R.Rewards = o.R.Rewards[:ln-1]
			break
		}
	}
...
func removeRewardsFromUsersSlice(o *User, related []*Reward) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
